...
export default function Modal({ setmod }) {
  ...
  const handleUpload = async () => {
    try {
      ...
      const updatedRepos = [...context.user_details.repos, {
  ...
  <div className="relative max-w-3xl mx-auto bg-primary p-8 rounded-lg w-1/2 text-text-col border-2 border-neon">
    <div className="text-3xl font-extrabold text-center mb-6">Create A Repository</div>
    <div className="mb-6">
      ...
      <div className="flex space-x-4">
        <button
          className={`px-4 py-2 rounded ${
            repotype === "true" ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-700"
          }`}
        ...
        <button
          className={`px-4 py-2 rounded ${
            repotype === "false" ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-700"
          }`}
    ...
    <div className="flex justify-center space-x-4">
      ...
      <button
        className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ${
          (!repodetails.name || !repotype) && "opacity-50 cursor-not-allowed"
        }`}
